...
NOW = datetime.datetime.now().strftime(f"%Y-%m-%dT%H:%M")
...
def test_sunblock():
    df = process(fetch(COVENTRY))
    res = find_sun(df,1)
    assert res.start > datetime.datetime.now()

if __name__ == "__main__":
    if "test" in sys.argv:
        sys.exit(pytest.main(["sunblock.py","-vv"]))
    loc = COVENTRY
    data = fetch(loc)
    df = process(data)
